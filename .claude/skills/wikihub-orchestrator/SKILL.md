...
description: >
  ...
  统一生成 WikiHub Markdown，管理 WikiHub 状态文件，并可选归档 Cubox 卡片。
metadata:
  version: "1.0.0"
  author: orange
...
WikiHub 唯一特定的主控 skill。它本身不直接抓取任何外部内容，而是通过 subprocess 调用通用工具 skill 完成抓取、转录和归档。

## 职责

- 从 Cubox 拉取未归档卡片，或读取 `--queue` 指定的输入队列。
- 按 URL 域名路由到对应工具 skill。
- 维护 WikiHub 状态：`wikihub-exported.json`、`*-export-config.json`、`/tmp/wikihub-pending.json`。
...
- 更新 `wikihub-exported.json`，键为来源特定 ID（如 `wechat_<article_id>`、`bilibili_<bvid>`）。
